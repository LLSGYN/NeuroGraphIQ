...
from smore.ngdb.util import QueryInterface
from .query_parser import QueryParser
from .rdf_query import get_rdf_triples
...
def exec_query(model, entity_embedding, parser, id2ent, query_str):
...
def create_app(args):
    ...
    app.config['entity_emb'] = entity_embedding

    # rdf
    app.config['rdf_kg'] = get_rdf_triples(args.data_path)
    return app
    ...
    query_str0 = """
        SELECT ?o WHERE
        {
            :m.07s9rl0 :neg.film.film.genre ?u .
            ?u :neg.film.director.film ?o .
        }
    """
    # 这里 :m.02hrh1q :neg.people.person.profession ?p . 会导致parser出错，需要debug
    query_str = """
        SELECT ?p WHERE
        {
            {
                SELECT ?p WHERE {
                    { :m.09c7w0 :neg.people.person.nationality ?p . }
                    UNION
                    { :m.07ssc :neg.people.person.nationality ?p . }
                }
            }
            :m.02hrh1q :neg.people.person.profession ?p .
        }
    """
    query_str1 = """
        SELECT ?m WHERE
        {
            ...
            ?p :pos.film.director.film ?m .
        }
    """
    # res = exec_query(model, entity_embedding, parser, id2ent, query_str)
    # print(res[:50])
...
start_args = r"""
    --gpus 0
    --data_path /home/lousy/smore/data/FB15k --eval_path /home/lousy/smore/data/FB15k/eval-betae
    ...
    --filter_test
    --share_optim_stats
    --online_sample --prefix /home/lousy/smore/smore/training/logs --online_sample_mode (500,0,w,wstruct,120)
    ...
    --checkpoint_path /home/lousy/smore/smore/training/logs/FB15k/1p.2p.3p.2i.3i-1p.2p.3p.2i.3i.ip.pi.2u.up/box/g-24.0-mode-(none,0.02)-ngpu-0.1.2.3-os-(500,0,w,wstruct,120)-(0.25,0.25,0.25,0.12,0.12)-dataset-(single,3000,e,True,before)-opt-(aggr,adam,cpu,False,5)-sharen-naive-lr_none/2023.12.09-21:27:41/
"""

torch.multiprocessing.set_start_method('spawn')
app = create_app(parse_args(start_args))
CORS(app)
...
def exec_rdflib_query(rdf_graph, query_str):
    ...
    processed = 'PREFIX : <http://rdf.freebase.com/ns/> \n' + query_str
...
def padding_length(r1, r2):
    n, m = len(r1), len(r2)
    res = [[None, None] for _ in range(max(n, m))]
...
def get_kg_options():
    ...
    if qtype == 'node':
        with open(os.path.join('/home/lousy/smore/data/FB15k', 'id2ent.pkl'), 'rb') as f:
        ...
        with open(os.path.join('/home/lousy/smore/data/FB15k', 'id2rel.pkl'), 'rb') as f:
            id2rel = pickle.load(f)
        relations = [id2rel[x] for x in id2rel]
...
def hande_query():
    ...
    res = exec_query(model, entity_embedding, parser, id2ent, query_str)
    res_rdflib = exec_rdflib_query(app.config['rdf_kg'], query_str)
    t2 = time.time()
    print(f"t1 - t0: {t1 - t0}")
    print(f"t2 - t1: {t2 - t1}")
    result_table = padding_length(res[:50], res_rdflib)
